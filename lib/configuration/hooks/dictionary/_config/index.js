  ...
  return {
    ...
    initialize: (callback) => {
      async.auto({
        ...
        'config/envs/*': (cb) => {
          dictionary.optional({
            dirname: path.resolve(restapi.config.rootPath, restapi.config.paths.config, 'envs'),
            excludeDirs: /(extension|extensions)$/,
            filter: /(.+)\.(js|json)$/,
            identity: false,
            depth: 1,
          }, cb);
      ...
      (err, config) => {
        ...
        restapi.config = _.merge(restapi.config, config['config/*'], config['config/envs/*'][restapi.config.env], pkg);
